...
class WeaponSystem {
  ...
  // View-model wall/pillar clip prevention. The gun sits offset from
  // the camera (forward + to the side) so it can end up closer to a
  // wall than the camera itself ever gets - the player's collision
  // radius only keeps the CAMERA a safe distance from geometry, not
  // the gun hanging out in front of it. This casts one short ray per
  // frame from the camera to the gun's current world position; if
  // temple geometry sits in between, the gun's local offset is pulled
  // back toward the camera (never behind it) so it can't render
  // poking through the wall. One raycast per frame against the same
  // static collider list already used elsewhere - cheap.
  avoidClipping(dt, staticColliders){
    if(!staticColliders || staticColliders.length === 0) return;
    if(!this._clipRaycaster) this._clipRaycaster = new THREE.Raycaster();
    if(this._clipFactor === undefined) this._clipFactor = 1;

    const camPos = this._clipCamPos || (this._clipCamPos = new THREE.Vector3());
    const gunPos = this._clipGunPos || (this._clipGunPos = new THREE.Vector3());
    this.camera.getWorldPosition(camPos);
    this.gunGroup.getWorldPosition(gunPos);

    const dir = gunPos.clone().sub(camPos);
    const fullDist = dir.length();
    let targetFactor = 1;
    if(fullDist > 0.02){
      dir.normalize();
      this._clipRaycaster.set(camPos, dir);
      this._clipRaycaster.far = fullDist;
      const hits = this._clipRaycaster.intersectObjects(staticColliders, false);
      if(hits.length > 0){
        // Stop a little short of the actual hit point so the gun's own
        // thickness never touches the wall surface.
        targetFactor = Math.max(0.2, (hits[0].distance / fullDist) - 0.12);
      }
    }
    // Smoothed, not snapped - an instant jump reads as a glitch/pop.
    this._clipFactor += (targetFactor - this._clipFactor) * Math.min(1, dt*16);
    this.gunGroup.position.x *= this._clipFactor;
    this.gunGroup.position.z *= this._clipFactor;
  }
}
