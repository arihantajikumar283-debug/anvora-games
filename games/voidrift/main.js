...
class Game {
  ...
  async init(){
    ...
    // Presentation-only resize plumbing: none of these listeners touch
    // game state (level/health/score/position) - they only re-fit the
    // renderer/camera to whatever viewport the browser is currently
    // giving us (rotation, fullscreen enter/exit, iOS address-bar show/
    // hide, etc). See _onResize().
    window.addEventListener('resize', ()=> this._onResize());
    window.addEventListener('orientationchange', ()=> setTimeout(()=>this._onResize(), 60));
  ...
  _onResize(){
    ...
    // (excludes the address bar / home-indicator strip that innerWidth/
    // innerHeight don't always account for) - falls back to
    // window.innerWidth/innerHeight wherever it's unavailable.
  ...
  // Requests fullscreen + landscape lock. Only ever called on mobile,
  // and only ever from inside a click-handler call stack (startNewGame/
  // continueGame/respawnAtCheckpoint/nextLevel are all bound directly to
  // button onclick handlers), which is required for browsers to honor
  // the Fullscreen API. Every step is wrapped so an unsupported or
  // denied request just falls through silently - gameplay must never
  // be blocked or altered by this failing. The existing portrait
  // "ROTATE YOUR DEVICE" overlay (UI.checkOrientation) already covers
  // devices/browsers where the orientation lock isn't supported (e.g.
  // iOS Safari), so no separate fallback message is needed here.
  _enterFullscreenLandscape(){
    ...
    try{
      ...
      if(!alreadyFullscreen && reqFS){
        const p = reqFS.call(el);
        if(p && typeof p.catch === 'function') p.catch(()=>{});
      }
    }catch(e){ /* Fullscreen API unsupported/denied - continue in normal viewport */ }
    ...
    }catch(e){ /* Orientation lock unsupported - rotate-device overlay handles it */ }
  }
  ...
  _enterPlaying(){
    ...
    UI.showScreen(null);
    UI.showHUDAndMobile();
    this._enterFullscreenLandscape();
    UI.checkOrientation();
    const canvas = this.renderer.domElement;
  ...
  _loop(){
    ...
    if(this.state === 'playing'){
      ...
      this.weapons.avoidClipping(dt, this.level.staticColliders);
